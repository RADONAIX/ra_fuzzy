"""
RADONaix Recon MVP - Layer 2a: Interval Type-2 Fuzzy Core

Minimal, dependency-free IT2 implementation:
  - IT2 trapezoidal membership functions (upper MF + lower MF = FOU)
  - Rule firing with min t-norm -> firing interval [f_lower, f_upper]
  - Nie-Tan closed-form type reduction (MVP choice; KM can replace later)
...
class Rule:
    ...
    def fire(self, inputs: dict[str, float], vocab: dict[str, dict[str, IT2Trap]]) -> tuple[float, float]:
        f_lo, f_up = 1.0, 1.0
...
def nie_tan(
    ...
    for r in rules:
        ...
        num_r += f_up * c_r
        den_r += f_up
        firings.append({"rule": r.antecedents, "consequent": r.consequent, "f": (round(f_lo, 3), round(f_up, 3))})
...
def jaccard_it2(a: IT2Trap, b: IT2Trap, domain: tuple[float, float] = (0.0, 100.0), n: int = 501) -> float:
